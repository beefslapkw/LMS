export const reactivateBook = async(book_id, refreshDisplay) => {
    ...
    modalFooter.querySelector(".confirm-reactivate").addEventListener('click', async() => {
        const result = await reactivateBookDetails(book_id);

        if(result == 1){
            refreshDisplay();
            alert("Successfully reactivated book");
            myModal.hide();
        }
        else{
            alert(result);
...
const reactivateBookDetails = async(book_id) => {
    const jsondata = {
        book_id: book_id,
    };

    const formData = new FormData();
    formData.append("operation", "reactivateBook");
